  ...
  try {
    ...
    for (let i = 0; i < MAX_CHUNKS && usdcVaults.length < 5; i++) {
      ...
      try {
        ...
        const chunkUsdcVaults = logs.filter(l => l.args.asset?.toLowerCase() === USDC.toLowerCase())
        usdcVaults.push(...chunkUsdcVaults)
        
        if (chunkUsdcVaults.length > 0) {
          console.log(`Found ${chunkUsdcVaults.length} USDC vaults in this chunk`)
        }
      } catch (chunkError) {
        console.log(`Error searching chunk ${fromBlock}-${toBlock}:`, chunkError.message)
        continue
      }
    }
